...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-7">
      {/* Greeting banner */}
      <div className="card p-7 bg-gradient-to-r from-primary-600 to-primary-800 text-white border-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold">Welcome back, Admin 👋</h2>
            <p className="text-primary-100 text-sm mt-1.5">{today}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="card p-6 lg:col-span-2">
          ...
          {stats.recent_employees.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-10">No employees yet</p>
          ) : (
            <div className="space-y-3">
              {stats.recent_employees.map((emp) => (
                <div key={emp.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50">
