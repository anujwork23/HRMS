...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-200 flex items-center px-5 lg:px-8 gap-4">
      {/* Mobile menu button */}
      <button
        onClick={onMenuClick}
        className="lg:hidden p-2 rounded-xl text-gray-500 hover:bg-gray-100 -ml-1"
      >
        <Menu size={20} />
      </button>
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="relative p-2.5 rounded-xl text-gray-500 hover:bg-gray-100">
          <Bell size={18} />
          <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-primary-600 rounded-full" />
        </button>
      </div>
    </header>
  );
